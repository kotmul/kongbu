...
class WeightSyncWorkerExtension:
    ...
    def update_named_param(self, name: str, dtype: str, shape: Sequence[int]) -> None:
        ...
        if hasattr(self, "scorer_worker") and hasattr(self, "proposer_worker"):
            # If this is a SpecDecodeWorker, we need to update both the scorer and the proposer.
            # We try to update both, assuming that the weight name is present in both models or handled gracefully.
            # Note: vLLM's load_weights typically filters out weights that don't match, or we might need to be more specific.
            # For now, we attempt to load into both.
            self.proposer_worker.model_runner.model.load_weights(weights=[(name, weight)])
            print(f"[DEBUG] Updated weight '{name}' in draft model (proposer_worker).", flush=True)
        else:
...
def create_spec_worker_with_extension(*args, **kwargs):
    # Patch sd_worker_cls if it is "auto" because create_spec_worker expects a qualified name
    vllm_config = kwargs.get("vllm_config")
    if vllm_config and vllm_config.parallel_config.sd_worker_cls == "auto":
        vllm_config.parallel_config.sd_worker_cls = "vllm.worker.worker.Worker"

    from vllm.spec_decode.spec_decode_worker import create_spec_worker
    worker = create_spec_worker(*args, **kwargs)

    # Dynamically mixin WeightSyncWorkerExtension
    cls = worker.__class__
    if WeightSyncWorkerExtension not in cls.__bases__:
        # Create a new class that inherits from both
        # We use the original class name to avoid confusing vLLM logs/checks if any
        new_cls = type(cls.__name__, (cls, WeightSyncWorkerExtension), {})
        worker.__class__ = new_cls
...
class ScriptArguments:
    ...
    spec_model: str | None = field(
        default=None,
        metadata={"help": "The name of the draft model to be used in speculative decoding."},
    )
    spec_num_speculative_tokens: int | None = field(
        default=None,
        metadata={"help": "The number of speculative tokens to sample from the draft model in speculative decoding."},
    )
    spec_acceptance_method: str | None = field(
        default=None,
        metadata={
            "help": "Number of tensor parallel workers to use for the draft model. "
            "If not specified, it defaults to the same value as `tensor_parallel_size`."
        },
    )
    spec_top_k: int | None = field(
        default=None,
        metadata={
            "help": "Number of tensor parallel workers to use for the draft model. "
            "If not specified, it defaults to the same value as `tensor_parallel_size`."
    ...
    spec_disable_mqa_scorer: bool | None = field(
        default=None,
        metadata={
            "help": "Number of tensor parallel workers to use for the draft model. "
            "If not specified, it defaults to the same value as `tensor_parallel_size`."
...
def llm_worker(
    ...
    worker_cls = None
    worker_extension_cls = "kongbu.spec_vllm_seerve.WeightSyncWorkerExtension"

    if script_args.spec_model is not None:
        logging.info(f"Speculative decoding enabled with model: {script_args.spec_model}")
        speculative_config = {
            "model": script_args.spec_model,
            "num_speculative_tokens": script_args.spec_num_speculative_tokens,
            "acceptance_method": script_args.spec_acceptance_method,
            "top_k": script_args.spec_top_k,
            "disable_mqa_scorer": script_args.spec_disable_mqa_scorer,
        }
        # When speculative decoding is enabled, the worker_cls is a function (create_spec_worker),
        # so vLLM's default extension injection fails. We use a custom factory instead.
        worker_cls = "kongbu.spec_vllm_seerve.create_spec_worker_with_extension"
